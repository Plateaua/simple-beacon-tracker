...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, performFetchWithCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        EventLogFileManager.saveLocal(logs: [EventLog(_description: "doBackgoundFetch!", _detectedAt: Date())])
        UserNotificationManager.removeLocalNotifications(identifier: "fuck")
        UserNotificationManager.doLocalNotification(message: "doBackgoundFetch\(self.beaconService)", identifier: "fuck")
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {        
        EventLogFileManager.saveLocal(logs: [EventLog(_description: "applicationDidEnterBackground", _detectedAt: Date())])
        self.dobackgroundTask()
    }
    
    func dobackgroundTask(){
        if self.beaconService == nil{
        ...
        if let deviceToken = UserDefaults.standard.string(forKey: "deviceToken") {
            if let serviceToken = UserDefaults.standard.string(forKey: "serviceToken"){
                self.beaconService?.startServices(proximityUUID: ProximityUUID, deviceToken: deviceToken, serviceToken: serviceToken, success: {() -> Void in
                    ...
                    EventLogFileManager.saveLocal(logs: [EventLog(_description: "dobackgroundTask: found", _detectedAt: Date())])
                    print("found!")
                })
            }
        }
